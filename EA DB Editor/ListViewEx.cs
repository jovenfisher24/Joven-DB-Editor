...
namespace ListViewEx
{
	...
	public class ListViewEx	: System.Windows.Forms.ListView
	{
		...
		public event SubItemEventHandler ListClicked;
		...
		protected override void OnMouseUp(System.Windows.Forms.MouseEventArgs e)
		{
			...
			ListClickedAt(new Point(e.X, e.Y), e.Button);
			...
			EditSubitemAt(new Point(e.X, e.Y), e.Button);
		}
		...
		private void EditSubitemAt(Point p, MouseButtons mb)
		{
			ListViewItem item;
			int idx = GetSubItemAt(p.X, p.Y, out item);
			if (idx >= 0)
			{
				OnSubItemClicked(new SubItemEventArgs(item, idx, mb ) );
			}
		}
		private void ListClickedAt(Point p, MouseButtons mb)
		{
			ListViewItem item;
			int idx = GetSubItemAt(p.X, p.Y, out item);
			if (idx >= 0)
			{
				OnListClicked(new SubItemEventArgs(item, idx, mb));
			}
		}
		...
		{
			OnSubItemBeginEditing(new SubItemEventArgs(Item, SubItem, MouseButtons.None));
	...
	{
		public SubItemEventArgs(ListViewItem item, int subItem, MouseButtons mb)
		{
			_subItemIndex = subItem;
			_item = item;
			_mb		= mb;
		}
		private MouseButtons _mb = MouseButtons.None;
	...
	public class SubItemEndEditingEventArgs : SubItemEventArgs
	{
		...
		public SubItemEndEditingEventArgs(ListViewItem item, int subItem, string display, bool cancel) :
			base(item, subItem, MouseButtons.None)
		{
			_text = display;
			_cancel = cancel;
		}
	...
	// helper stuff to turn on double buffering and prevent flickering
